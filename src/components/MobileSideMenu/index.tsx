...
export function MobileSideMenu() {
  ...
  return (
    <>
      <motion.div
        className="absolute z-40 bg-black w-screen h-screen left-0 top-0"
        aria-modal="true"
        initial={{ opacity: 0 }}
        animate={isOpen ? { opacity: 0.3 } : { opacity: 0 }}
        onClick={() => switchVisibility(false)}
      ></motion.div>
      <motion.div className="absolute z-50 top-0 right-0 ml-auto bg-white shadow-md h-screen w-[220px] p-4">
        <div
          onClick={() => switchVisibility(false)}
          className="w-fit relative ml-auto p-2 rounded-md hover:bg-lightgray transition-colors duration-200"
        >
          <FiX size={24} />
        </div>
        ...
        <div className="border-b border-lightgray pb-4 mb-4">
          ...
          <ul className="flex flex-col items-start gap-2">
            <Link
              href={'#'}
              className="buttons-outline text-md font-semibold text-gray rounded-lg hover:text-black transition-colors duration-200 cursor-pointer flex items-center gap-2"
            >
              <FiInfo size={16} />
              About
            </Link>
